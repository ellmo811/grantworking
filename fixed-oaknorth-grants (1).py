...
total_grant_shares = st.sidebar.number_input(
    "Total Grant Shares",
    min_value=1,
    max_value=500000,  # Updated maximum to 500,000
    value=100000,
    step=100,
    ...
    with col1:
        for year in first_half:
            ...
            vested_shares_input[year] = st.number_input(
                f"{year}", 
                min_value=0, 
                max_value=min(500000, int(total_grant_shares)),  # Updated maximum to 500,000
                value=default_value, 
                step=100,
                key=f"vest_{year}"
            )
...
try:
    ...
    try:
        ...
        pbt_growth_rate = 0.10
        common_redemption_rate = 0.05
        option_redemption_rate = 0.05
        results = calculate_results(pbt_growth_rate)
...
if total_common_shares > 0:
    with tab1:
        ...
        for rate in redemption_rates:
            # Calculate with the new rate
            rate_results = calculate_results(0.20, rate, None)  # Fix PBT growth at 20%
...
with tab2:
    ...
    # Calculate for various redemption rates
    redemption_rates = [0.00, 0.05, 0.10, 0.15, 0.20]
    sensitivity_data = []
    
    for rate in redemption_rates:
        # Create a backup of the original rate
        original_rate = option_redemption_rate
        
        # Temporarily set the rate for calculation
        global option_redemption_rate
        option_redemption_rate = rate
        
        # Calculate with the new rate
        rate_results = calculate_results(fixed_growth)
        
        # Restore the original rate
        option_redemption_rate = original_rate
